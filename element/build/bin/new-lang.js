...
// const configPath = path.resolve(__dirname, '../../documents/i18n', lang);

// 添加到 components.json
const componentFile = require('../../documents/i18n/component.json');
...
fileSave(path.join(__dirname, '../../documents/i18n/component.json'))
  .write(JSON.stringify(componentFile, null, '  '), 'utf8')
  .end('\n');

// 添加到 page.json
const pageFile = require('../../documents/i18n/page.json');
let pageNew = Object.assign({}, pageFile.filter(item => item.lang === 'en-US')[0], { lang });
pageFile.push(pageNew);
fileSave(path.join(__dirname, '../../documents/i18n/page.json'))
  .write(JSON.stringify(pageFile, null, '  '), 'utf8')
  .end('\n');

// 添加到 route.json
const routeFile = require('../../documents/i18n/route.json');
routeFile.push({ lang });
fileSave(path.join(__dirname, '../../documents/i18n/route.json'))
  .write(JSON.stringify(routeFile, null, '  '), 'utf8')
  .end('\n');

// 添加到 nav.config.json
const navFile = require('../../documents/nav.config.json');
...
// docs 下新建对应文件夹
try {
  fs.statSync(path.resolve(__dirname, `../../documents/docs/${ lang }`));
} catch (e) {
  fs.mkdirSync(path.resolve(__dirname, `../../documents/docs/${ lang }`));
}
